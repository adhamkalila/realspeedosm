...
road_path = 'roads.osm'
newwayID_seed = 1000 
output_file_name = 'output.osm'

# 1 #### fill in the path for the speed dataset:


### Speed Data and units
avgspeed = '/speed.csv'
...
def CheckNodesForSpeeds (wayID, originalway, units, verbose = 'no'):
    ...
    for nodes in relevantSpeedsShort.iterrows():
        ...
        elif nodelistfloat.index(endnode) < nodelistfloat.index(startnode): # reverse sequence
...
#                 samewayID = True
    ...
    while len(nodepairdf[nodepairdf.end == endPos]) > 0:  
        ...
        for i in range(start+1, end+1): # delete nodes in orig way from start +1 to endPos
            nodeID = nodelist[i]
            try: originalway.remove(originalway.find("./nd/[@ref='"+nodeID+"']"))
            except: pass
            if verbose == 'yes': print('removed node index',i)
    ...
    copypairs =[] # this list will keep track of the segments of the original way that will be retained
    ...
    for i, pair in enumerate(sortedNodepairdf): 
        ...
        for i in range(pair[0]+1, pair[1]): # delete nodes in orig way from node start +1 to node end -1
            nodeID = nodelist[i]
            try: originalway.remove(originalway.find("./nd/[@ref='"+nodeID+"']"))
            except: pass
            if verbose == 'yes': print('removed node index',i)
...
for i,originalway in enumerate(root.findall("./way")):
    ...
    # Suggestion: Delete the original way that was hollowed out and left with non sequential nodes greter than 1 in count
